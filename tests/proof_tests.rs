...
fn test_sender_addr() -> Vec<u8> { vec![0u8; 32] }
fn test_token_addr() -> Vec<u8> { vec![0u8; 32] }
...
fn transfer_sigma_proof_serialize_deserialize_roundtrip_no_auditors() {
    ...
    let decoded = ConfidentialTransfer::deserialize_sigma_proof(&bytes).expect("deserialize should succeed");
